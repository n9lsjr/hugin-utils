...
function solveSync (blob, targetHex, jobId, startNonce, timeBudgetMs, cancel, cancelIndex) {
...
async function solveAsync (blob, targetHex, jobId, startNonce, timeBudgetMs, cancel, cancelIndex) {
  const offset = getNonceOffset(blob)
  const cancelArr = new Int32Array(cancel)
  const deadline = Date.now() + timeBudgetMs
  let nonce = startNonce >>> 0
...
parentPort.on('message', async (msg) => {
  ...
  try {
    const solveFn = hashSync ? solveSync : solveAsync
    const share = hashSync
      ? solveFn(msg.blob, msg.target, msg.jobId, msg.startNonce, msg.timeBudgetMs, msg.cancel, msg.cancelIndex)
      : await solveFn(msg.blob, msg.target, msg.jobId, msg.startNonce, msg.timeBudgetMs, msg.cancel, msg.cancelIndex)
